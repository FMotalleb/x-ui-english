...
download_xui(){
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://api.github.com/repos/fmotalleb/x-ui-english/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/') || last_version=$(curl -sm8 https://raw.githubusercontent.com/fmotalleb/x-ui-english/main/config/version >/dev/null 2>&1)
        ...
        last_version=$1
        url="https://github.com/fmotalleb/x-ui-english/releases/download/${last_version}/x-ui-linux-$(archAffix).tar.gz"
...
install_xui() {
    ...
    echo -e "Niduka Akalanka   - https://github.com/NidokaAkalan"
    echo -e "Motalleb Fallahnezhad   - https://github.com/FMotalleb"
    echo -e "--------------------------------------------------------------------------------"
    show_login_info
    echo -e ""
